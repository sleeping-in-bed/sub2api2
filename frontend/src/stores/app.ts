...
export const useAppStore = defineStore('app', () => {
  ...
  const siteName = ref<string>('MindAI')
  ...
  const binaryUpdateEnabled = ref<boolean>(false)
  ...
  async function fetchVersion(force = false): Promise<VersionInfo | null> {
    ...
    if (versionLoaded.value && !force) {
      return {
        ...
        binary_update_enabled: binaryUpdateEnabled.value,
    ...
    try {
      ...
      binaryUpdateEnabled.value = data.binary_update_enabled
  ...
  function applySettings(config: PublicSettings): void {
    ...
    siteName.value = config.site_name || 'MindAI'
