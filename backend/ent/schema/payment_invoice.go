...
func (PaymentInvoice) Annotations() []schema.Annotation {
	return []schema.Annotation{entsql.Annotation{Table: "payment_invoices"}}
}

func (PaymentInvoice) Fields() []ent.Field {
	return []ent.Field{
		field.Int64("user_id"),
		field.String("title_name").MaxLen(200),
		field.String("tax_id").MaxLen(32),
		...
		field.String("storage_provider").MaxLen(20).Default("local"),
		field.String("storage_key").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "text"}),
		field.String("file_name").Optional().Nillable().MaxLen(255),
		field.String("content_type").Optional().Nillable().MaxLen(100),
...
func (PaymentInvoice) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.From("user", User.Type).Ref("payment_invoices").Field("user_id").Unique().Required(),
	}
}
