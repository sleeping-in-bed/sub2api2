...
// The PaymentInvoiceFunc type is an adapter to allow the use of ordinary
// function as PaymentInvoice mutator.
type PaymentInvoiceFunc func(context.Context, *ent.PaymentInvoiceMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PaymentInvoiceFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PaymentInvoiceMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PaymentInvoiceMutation", m)
}
