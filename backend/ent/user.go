...
type UserEdges struct {
	...
	// PaymentInvoices holds the value of the payment_invoices edge.
...
// PaymentInvoicesOrErr returns the PaymentInvoices value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) PaymentInvoicesOrErr() ([]*PaymentInvoice, error) {
	if e.loadedTypes[10] {
		return e.PaymentInvoices, nil
	}
	return nil, &NotLoadedError{edge: "payment_invoices"}
}
...
// QueryPaymentInvoices queries the "payment_invoices" edge of the User entity.
func (_m *User) QueryPaymentInvoices() *PaymentInvoiceQuery {
	return NewUserClient(_m.config).QueryPaymentInvoices(_m)
}
